...
TARGET = {"id": "99", "name": "load_config", "file_path": "utils/config_loader.py"}
...
def test_bfs_finds_direct_neighbor():
    ...
    path = BoundedBFS(client).find_path(ENTRY, TARGET, max_hops=5)
...
def test_bfs_respects_max_hops():
    ...
    assert BoundedBFS(client).find_path(ENTRY, TARGET, max_hops=2) is None
...
def test_bfs_returns_none_when_no_path():
    ...
    assert BoundedBFS(client).find_path(ENTRY, TARGET, max_hops=5) is None
...
def test_bfs_handles_cycles():
    ...
    assert BoundedBFS(client).find_path(ENTRY, TARGET, max_hops=10) is None
...
def test_bfs_returns_start_when_entry_is_target():
    client = MagicMock()
    path = BoundedBFS(client).find_path({"id": "99", "path": "x"}, TARGET, max_hops=5)
    assert path is not None
    assert len(path) == 1
