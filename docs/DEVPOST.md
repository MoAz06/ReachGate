...
1. **Python engine** (`src/reachgate/`) — Orbit REST client, bounded BFS over `neighbors` queries with termination reporting (why each walk stopped), deterministic policy engine with three verdicts, reachability certificates, GitLab actions (work items, MR comments, JSON artifact), and a portable evidence layer: standards-aligned OpenVEX and SARIF 2.1.0 exports, a sha256 evidence manifest, an offline cross-checking verifier, a package-safe `reachgate` CLI with a coverage/blind-spot report, a portable evidence capsule, optional Ed25519 signing/tamper detection, derived fix-verification proof with markdown output, changed-file/path-overlap blame (not causation), an offline evidence explorer, adversarial selftest, a machine-checkable Evidence Contract validator, and fake-green rejection demo fixtures. 422 focused tests (+5 marked standalone install-gate tests deselected by default).
...
| MR !3 first run + rerun | Shows the workflow is idempotent: first run creates receipt comments, rerun logs `unchanged`, comment count stays 2, artifact uploads again, and no work item is created by the MR flow. |
...
- **The evidence is portable and falsifiable.** ReachGate is an offline-verifiable evidence layer, not just a scanner. The receipts are exported as standards-aligned [OpenVEX](https://github.com/openvex/spec/blob/main/OPENVEX-SPEC.md) for CVE/SCA (`REACHABLE` → `affected`, exhaustive `NOT_REACHABLE` → `not_affected` with justification `vulnerable_code_not_in_execute_path`, `UNKNOWN` → `under_investigation` — and **never** `not_affected` from a search that hit a bound or errored) and as **SARIF 2.1.0** for SAST/code-flow (a REACHABLE path becomes a SARIF `codeFlow`/`threadFlow`; NOT_REACHABLE stays "within configured bounds"; UNKNOWN is a typed gap, never safe). A sha256 **evidence manifest** ties the machine-readable artifacts together, and `tools/verify_proof.py` cross-checks the OpenVEX and SARIF back against the same receipts. Most reachability tools ask you to trust a verdict; ReachGate ships evidence downstream tooling can consume and check. See the [Judge Pack](JUDGE_PACK.md).
...
- **One command, package-safe.** A `reachgate` CLI wraps the offline surface — `selftest`, `coverage`, `verify`, `export-vex`, `export-sarif`, `manifest`, `proof`, `policy explain`, `contract-check`, `fixcheck`, `blame`, `explorer`, `judge`, and `capsule build` — with `[project.scripts] reachgate = "reachgate.cli:main"`. `reachgate selftest` is the fastest "do not trust us, prove it" check: real evidence passes, fake-green evidence fails for the right reason, UNKNOWN remains an evidence gap, and the signature/tamper leg runs when installed with the optional `sign` extra (otherwise it is skipped honestly). `reachgate coverage` is a blind-spot report (verdict counts, UNKNOWN reasons with typed next actions, missing-code-anchor findings, honest limitations) in text/json/html, derived only from the receipts. `reachgate judge` runs the whole offline pipeline in one command; `reachgate policy explain` prints the recorded policy with honest provenance. `scan` is deliberately live-only (it needs Orbit + a token) and documented as such, so the CLI never fakes a result. See [docs/GITLAB_CI_SETUP.md](GITLAB_CI_SETUP.md) and [docs/INTEGRATIONS.md](INTEGRATIONS.md).
- **Portable evidence capsule.** `reachgate capsule build` produces a deterministic, gitignored `dist/reachgate-evidence-capsule.zip` bundling the receipts, OpenVEX, SARIF, evidence manifest, judge-proof HTML, Judge Pack, and a `HOW_TO_VERIFY.txt`. A reviewer can take the capsule away and confirm the manifest hashes offline. Most scanners output findings; ReachGate outputs a portable evidence capsule.
