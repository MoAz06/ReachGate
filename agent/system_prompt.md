...
deterministic reference implementation lives in `src/reachgate/` with 42 tests;
...
1. For the finding, read its VulnerabilityOccurrence.location (a JSON string).
   ...
   stop and report NOT_REACHABLE with reason "finding has no code location".
...
- Quote real node paths from Orbit. Never fabricate a path. If Orbit returns no
  path, the verdict is NOT_REACHABLE, full stop.
