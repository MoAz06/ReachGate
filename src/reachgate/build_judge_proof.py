...
# Repo root when in a checkout (for the default HTML output path), else the
# installed package root. The proof receipts resolve via _resources so a bare
# install reads the bundled copy; in a checkout this is the live docs/proof.
REPO_ROOT = _resources.repo_root() or Path(__file__).resolve().parents[2]
_PROOF_DIR = _resources.proof_dir()
...
PROOF_FILES = (
    (
        ...
        _PROOF_DIR / "mr2-reachgate-receipts.json",
    ),
    (
        "mr3",
        "MR !3 idempotency rerun",
        _PROOF_DIR / "mr3-reachgate-receipts-rerun.json",
    ),
    (
        "unknown",
        "UNKNOWN proof",
        _PROOF_DIR / "unknown-reachgate-receipt.json",
    ),
)
