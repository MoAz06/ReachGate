...
Packaging note:
  Canonical home of the verifier; ``tools/verify_proof.py`` is a thin shim
  re-exporting from here. The proof directory resolves via
  :mod:`reachgate._resources` (docs/proof in a checkout, the bundled
  package-data copy after a bare ``pip install``).
...
# Import the VEX/SARIF mappings so the cross-checks use the exact same logic
# that produced the documents -- no second, drifting copy of the rules.
from . import export_vex
from . import export_sarif

_PROOF_DIR = str(_resources.proof_dir())
MR2 = os.path.join(_PROOF_DIR, "mr2-reachgate-receipts.json")
MR3 = os.path.join(_PROOF_DIR, "mr3-reachgate-receipts-rerun.json")
UNKNOWN = os.path.join(_PROOF_DIR, "unknown-reachgate-receipt.json")
VEX = os.path.join(_PROOF_DIR, "reachgate.openvex.json")
SARIF = os.path.join(_PROOF_DIR, "reachgate.sarif.json")
